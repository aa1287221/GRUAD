...
def generate_output(args, epoch, model, gen_dataset, disp_uncertainty=True, startPoint=0, endPoint=10000):
    if args.save_fig:
        ...
        target = preprocess_data.reconstruct(
            gen_dataset.cpu(), TimeseriesData.mean, TimeseriesData.std)
        outSeq = preprocess_data.reconstruct(
            outSeq, TimeseriesData.mean, TimeseriesData.std)
        # if epoch>40:
...
def train(args, model, train_dataset, epoch):
    ...
    with torch.enable_grad():
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            loss = criterion(outSeq.contiguous().view(
                args.batch_size, -1), targetSeq.contiguous().view(args.batch_size, -1))
