    ...
    with torch.no_grad():
        ...
        for t in range(len(train_dataset)):
            out, hidden = model.forward(
                train_dataset[t].unsqueeze(0), pasthidden)
            predictions.append([])
            organized.append([])
            errors.append([])
    ...
    with torch.no_grad():
        ...
        for t in range(len(dataset)):
            ...
            if score_predictor is not None:
                predicted_scores.append(score_predictor.predict(
                    model.extract_hidden(hidden).numpy()))
            ...
            if t >= args.prediction_window_size:
                for step in range(args.prediction_window_size):
                    rearranged[t].append(
                        predictions[step + t - args.prediction_window_size][args.prediction_window_size - 1 - step])
                rearranged[t] = torch.FloatTensor(
                    rearranged[t]).to(args.device).unsqueeze(0)
                errors[t] = rearranged[t] - dataset[t][0][channel_idx]
            else:
                rearranged[t] = torch.zeros(
                    1, args.prediction_window_size).to(args.device)
                errors[t] = torch.zeros(
                    1, args.prediction_window_size).to(args.device)
...
def get_precision_recall(mean, cov, errors, args, score, label, num_samples, beta=1.0, sampling=None, predicted_score=None):
    ...
    if predicted_score is not None:
        score = score - \
            torch.FloatTensor(predicted_score).squeeze().to(args.device)
